...
1. **Download** the latest portable `.zip` from [GitHub Releases](https://github.com/OR-Beyond/modpack-exporter/releases)
...
- **Zero-dependency portable build** — extract and run, no installer needed
...
- **Auto-update** — checks GitHub Releases on launch and prompts for updates

---

## Requirements

- **Windows** (macOS/Linux builds available on request)
...
1. Download the latest `ORB.Modpack.Exporter-win32-x64-{version}.zip` from [Releases](https://github.com/OR-Beyond/modpack-exporter/releases)
2. Extract to any folder (e.g., `Desktop/ORB Exporter`)
3. Run `orb-modpack-exporter.exe`
...
| Auto-update | Custom portable updater (fetches latest `.zip` from GitHub Releases) |
...
This project uses [SignPath Foundation](https://signpath.org/) for free open-source code signing. Binaries are signed with an OV-level certificate verified against this repository.
